...
router.post('/proxy/chat', async (req, res) => {
  try {
    ...
    logger.info('代理请求到智谱API', { model, messagesCount: messages.length });
    ...
    if (!response.ok) {
      ...
      logger.error('智谱API请求失败', { status: response.status, error: errorText });
