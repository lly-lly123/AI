...
class AutoMaintainer {
  ...
  determineFixStrategy(bug) {
    ...
    if (similarBugs.length > 0) {
      // 使用历史成功的修复策略
      const successfulFixes = similarBugs
        .filter(b => b.status === 'fixed' && b.fixAttempts.some(f => f.status === 'success'))
        .map(b => b.fixAttempts.find(f => f.status === 'success').strategy);
      
      if (successfulFixes.length > 0) {
        // 选择最常用的成功策略
        const strategyCounts = {};
        successfulFixes.forEach(s => {
    ...
    const decision = decisionEngine.applyRules('systemHealth', {
      bugType: bug.type,
    ...
    if (decision.action !== 'no_action') {
      return this.mapDecisionToStrategy(decision.action);
    }
   ...
   */
  findSimilarBugs(bug) {
    return this.maintenanceLog.bugs.filter(b => 
      b.id !== bug.id &&
      b.type === bug.type &&
      this.calculateSimilarity(b.error.message, bug.error.message) > 0.7
    );
  }
   ...
   * 映射决策到策略
   */
  mapDecisionToStrategy(action) {
    const mapping = {
      'immediate_intervention': 'restart_service',
      'enable_fallback': 'fix_config',
      'optimize_or_cache': 'clear_cache',
      'cleanup_and_validate': 'clear_cache',
      'schedule_maintenance': 'fix_config'
    };
    return mapping[action] || 'restart_service';
  }
